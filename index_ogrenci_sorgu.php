...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $ogr_isim = $_POST['ogr_isim'];
    $ogr_sifre = $_POST['ogr_sifre'];
    ...
    if (isset($validCodes[$captcha_img]) && $dogrulama_kodu === $validCodes[$captcha_img]) {
        ...
        $sorgu = "SELECT tc, geldigi_okul FROM muaf_ogrenci WHERE geldigi_okul = '$ogr_isim' AND tc = '$ogr_sifre'";
        $sonuc = $conn->query($sorgu);

        if ($sonuc->num_rows > 0) {
              $satir = $sonuc->fetch_assoc();
            $_SESSION['tc'] = $satir['tc'];  // TC bilgisini oturuma kaydet

            header("Location: ogr/main.php");  // Oturum verisini kullanarak yönlendir
            exit();
        } else {
            // Hatalı giriş
           // echo "Yanlış TC veya şifre!";
           $hata = "Yanlış Okul veya Şifre!";
        }
    } else {
...
<center>
    <div class="form-container">
        ...
        <form action="" method="post">
            <select id="ogr_isim"  name="ogr_isim"  required>
                    <option value="">Üniversite Seçin</option>
                    <?php
                    $universiteler->data_seek(0);
                    while ($row = $universiteler->fetch_assoc()) {
                      
                            echo "<option value='{$row['universite_ad']}'>{$row['universite_ad']}</option>";
            ...
            <input type="password" id="ogr_sifre" name="ogr_sifre" placeholder="Şifre" required>
        ...
        <a href="index.php" class="link">Yönetici Sistemi Giriş</a>
